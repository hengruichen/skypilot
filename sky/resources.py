    ...
    * for provisioning on a cloud
    ...
    def __init__(
        ...
        Args:
          ...
          accelerator_args: the arguments for the accelerators.
          use_spot: whether to use a preemptible/vm instance.
          spot_recovery: the type of preemptible/vm recovery to use.
          region: the region to use.
          zone: the zone to use.
          image_id: the image ID to use. If a dict, must be a dict of the form
            ``{'us-west1': 'image-12345678901234567890123456789012'}``.
          disk_size: the size of the disk in GB.
          disk_tier: the tier of the disk. Can be 'high', 'medium', or 'low'.
        ...
        if isinstance(accelerators, str):
            accelerators = {
                ...
                acc_count
                for acc, acc_count in
                resources_utils.parse_accelerators(accelerators).items()
            }

        if isinstance(image_id, str):
            image_id = {region: image_id for region in self.regions}
        ...
        resources_fields = {
            ...
            '_accelerator_args': accelerator_args,
            '_use_spot': use_spot,
            '_spot_recovery': spot_recovery,
            '_region': region,
            '_zone': zone,
            '_image_id': image_id,
            '_disk_size': disk_size,
            '_disk_tier': disk_tier,
            '_ports': ports,
        ...
        if self._version != self._VERSION:
            raise ValueError(
                f'Invalid resource version: {self._version} != {self._VERSION}')
        ...
        if self._version < 6:
            accelerators = resources_fields['_accelerators']
            if accelerators is not None:
                accelerators = {
                    accelerator_registry.canonicalize_accelerator_name(acc):
                    acc_count for acc, acc_count in accelerators.items()
                }
            resources_fields['_accelerators'] = accelerators
        ...
        image_id = resources_fields.get('_image_id', None)
        if isinstance(image_id, str):
            resources_fields['_image_id'] = {resources_fields.get('_region',
                                                                   None):
                                             image_id}
        ...
        if self._version < 13:
            original_ports = resources_fields.get('_ports', None)
            if original_ports is not None:
                resources_fields['_ports'] = resources_utils.simplify_ports(
                    [str(port) for port in original_ports])

        self.__dict__.update(resources_fields)
